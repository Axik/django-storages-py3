...
that returns an image as result.
        ...
        self.connection = pyodbc.connect(
            'DRIVER=%s;SERVER=%s;DATABASE=%s;UID=%s;PWD=%s' % (self.DATABASE_ODBC_DRIVER, self.DATABASE_HOST, self.DATABASE_NAME,
    ...
    def _open(self, name, mode='rb'):
        ...
        row = self.cursor.execute("SELECT %s from %s where %s = '%s'" %
                                  (self.blob_column, self.db_table, self.fname_column, name)).fetchone()
        if row is None:
            return None
        ...
        if self.exists(name):
            self.cursor.execute(
                "UPDATE %s SET %s = ?, %s = ? WHERE %s = '%s'" % (
                    self.db_table, self.blob_column, self.size_column, self.fname_column, name),
                (binary, size))
        else:
    ...
    def exists(self, name):
        row = self.cursor.execute("SELECT %s from %s where %s = '%s'" %
                                  (self.fname_column, self.db_table, self.fname_column, name)).fetchone()
    ...
    def size(self, name):
        row = self.cursor.execute("SELECT %s from %s where %s = '%s'" %
                                  (self.size_column, self.db_table, self.fname_column, name)).fetchone()
        if row is None:
            return 0
        else:
            return int(row[0])
